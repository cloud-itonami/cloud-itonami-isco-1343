(ns aged-care.actor
  "AgedCareActor — the ISCO-08 1343 aged care services manager actor as
  a `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one administrative request for a
  facility (intake → advise → govern → decide → commit/hold, with a
  ...
  The unconditional invariant: the AgedCareAdvisor can never
  directly commit a record or dispatch a robot action the
  AgedCareGovernor refuses — every commit-record! call is gated
  behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [aged-care.advisor :as advisor]
            [aged-care.governor :as governor]
            [aged-care.store :as store]))

(defn build-graph
  "Build a compiled AgedCareActor graph. `store` implements
  `aged-care.store/Store`. `advisor` implements
  `aged-care.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:facility-id (:facility-id request)
