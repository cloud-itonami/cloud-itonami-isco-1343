(ns aged-care.advisor
  "AgedCareAdvisor — proposes an administrative action (schedule staffing,
  log resident record, draft family correspondence, flag an incident) for
  a registered facility. The advisor is swappable: `mock-advisor`
  ...
  PROPOSAL — it never writes to the store and has no notion of facility/
  resident provenance or incident-escalation risk; `aged-care.governor` is
  the independent system that decides whether the proposal may proceed, per
  the itonami actor pattern.

  A proposal is a map:
    {:op :schedule-staffing|:log-resident-record|:draft-family-correspondence|:flag-incident
...
(def ^:private system-prompt
  "You are an aged care facility administrative advisor. Given an
   administrative request (staffing, resident records, correspondence,
   incident logging), propose an :op, an honest :confidence (0.0-1.0),
   and a :stake (:low/:medium/:high). Never fabricate confidence you don't
   have. CRITICAL: Do NOT propose clinical decisions, medications, or
   treatment determinations — this actor is administrative support only.
   Licensed care staff retain all clinical authority.")
